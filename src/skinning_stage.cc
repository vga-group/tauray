...
skinning_stage::skinning_stage(device_mask dev, uint32_t max_instances)
:   multi_device_stage(dev),
    comp(dev, compute_pipeline::params{{"shader/skinning.comp"}, {
...
void skinning_stage::set_scene(scene* s)
{
    ...
    for(auto[dev, comp]: comp)
    {
        ...
        for(uint32_t i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i)
        {
            ...
            vk::CommandBuffer cb = begin_compute(dev.index);
